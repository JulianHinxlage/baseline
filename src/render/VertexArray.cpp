...
namespace baseline {
    ...
    void VertexArray::addIndexBuffer(const std::shared_ptr<Buffer> &indexBuffer, Type type) {
        if(id == 0){
    ...
    void VertexArray::addVertexBuffer(const std::shared_ptr<Buffer> &vertexBuffer, std::vector<Attribute> layout, int divisor) {
        if(id == 0){
            glGenVertexArrays(1, &id);
        }
        bind();
