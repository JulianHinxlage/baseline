...
namespace baseline {
    ...
    void Texture::setCubeMap(bool enableMipMapping) {
        if(type != TextureType::TEXTURE_CUBE_MAP && id != 0 && width != 0 && height != 0) {
            bind(0);
            int x = width / 4;
            int y = height / 3;
            int xs[] = {2, 0, 3, 1, 1, 1};
    ...
    void Texture::setData(Color* data, int count){
        if (count < width * height) {
            return;
        }
        glTexSubImage2D(internalEnum(type), 0, 0, 0, width, height, GL_RGBA, GL_UNSIGNED_BYTE, data);
    }
